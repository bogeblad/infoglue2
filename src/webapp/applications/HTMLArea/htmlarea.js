...
HTMLArea.Config = function () {
	...
	for (var i in this.btnList) {
		...
		//alert("_editor_url + this.imgURL + btn[1]:" + _editor_url + ":"  + this.imgURL + ":" + btn[1]);
			...
			HTMLArea._addEvents (       
				...
				function(event) { 
					if(doc.designMode !== 'on') { 
						if(HTMLArea.last_on != doc && HTMLArea.last_on != null) 
							HTMLArea.last_on.designMode = 'off'; 
							doc.designMode = 'on'; 
						} 
					HTMLArea.last_on = doc; 
					} 
				); 
		}
...
HTMLArea.prototype.insertHTML = function(html) {
	//alert("Inside insertHTML with:" + html);
	var sel = this._getSelection();
	var range = this._createRange(sel);
	//alert("sel:" + sel);
	//alert("range:" + range.text);
	//alert("range:" + range.htmlText);
	//alert("HTMLArea.is_ie:" + HTMLArea.is_ie);
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	switch (cmdID) {
	    ...
	    case "createinlinelink": this._createInlineLink();break;
	    case "popupeditor":
	    ...
	    case "insertinlineimage": this._insertInlineImage(); break;
...
HTMLArea.prototype._relativePopupDialog = function(url, action, init) {
