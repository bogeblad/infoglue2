...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "Verdana"; font-size: 8pt;';
...
//  ['strikethrough','subscript','superscript','separator'],
    ...
    ['HorizontalRule','Createlink','InlineLink','InlineImage','InsertImage','InsertTable',/*'htmlmode',*/'separator'],
...
function editor_action(button_id) {
  ...
  // InlineImage
  	...
  	if(document.location.href.indexOf("WYSIWYG") > -1)
	  	urlPrefix = "../../../";
  	
  	//alert("urlPrefix:" + urlPrefix);
  	openPopup(urlPrefix + "ViewContentVersion!viewAssetsDialog.action?contentId=" + contentId + "&languageId=" + languageId + "&textAreaId=" + editor_obj.id, "InlineImage", "toolbar=no,status=yes,scrollbars=yes,location=no,menubar=no,directories=no,resizable=no,width=500,height=400,left=200,top=200");
    textArea = editor_obj;
  }


  // InlineLink
  else if (cmdID == 'InlineLink') {
  	urlPrefix = "";
  	if(!contentId)
  		contentId = self.opener.contentId;
  	if(!languageId)
  	...
  	//alert("urlPrefix:" + urlPrefix);
  	openPopup(urlPrefix + "ViewStructureTreeForInlineLink.action?contentId=" + contentId + "&languageId=" + languageId + "&textAreaId=" + editor_obj.id, "InlineImage", "toolbar=no,status=yes,scrollbars=yes,location=no,menubar=no,directories=no,resizable=no,width=500,height=400,left=200,top=200");
    textArea = editor_obj;
  }
...
{ 
	...
	/*if (isTextarea) { return editor_obj.value; }*/ 
	if (isTextarea) { return obj.value; } 
	else { return editor_obj.contentWindow.document.body.innerHTML; } 
} 
...
{ 
	...
	/*OK, now we must copy html in the original textarea...*/ 
	var obj = document.all[objname]; 
	isTextarea = (obj.tagName.toLowerCase() == 'textarea'); 
	
	if (isTextarea) { obj.value = html; } 
	else { obj.contentWindow.document.body.innerHTML = html; } 
} 
