...
class ExternalSearchServiceDirectoryHandler
{
	...
	void clearOldDirectories()
	{
		...
		synchronized (oldReferences)
		{
			...
			Collections.sort(oldReferences, new Comparator<Object>()
			{
				@Override
				public int compare(Object o1, Object o2)
				{
					boolean isO1IndexSearcher = o1 instanceof IndexSearcher;
					boolean isO2IndexSearcher = o2 instanceof IndexSearcher;
					if (isO1IndexSearcher && isO2IndexSearcher)
					{
						return 0;
					}
					else if (isO1IndexSearcher && !isO2IndexSearcher)
					{
						return -1;
					}
					else if (!isO1IndexSearcher && isO2IndexSearcher)
				...
				try
				{
					...
					if (reference instanceof FSDirectory)
					{
						...
						else
						{
							...
							{
								logger.info("Failed to remove old directory from disk. Will keep it in the list. File: " + file.getAbsolutePath() + ". Message: " + ioex.getMessage());
							}
						}
					}
					else if (reference instanceof IndexSearcher)
					{
						...
						try
						{
							((IndexSearcher) reference).getIndexReader().close();
