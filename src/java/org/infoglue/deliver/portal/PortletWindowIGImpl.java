...
class PortletWindowIGImpl implements PortletWindowIG {
    ...
    public String render() throws PortalException {
        log.debug(
            "render("
                + renderWindow.getPortletEntity().getId()
                + ", "
                + renderWindow.getId()
                + ") invoked");
        try {
            // Create a buffered response to "catch" output from rendering the portlet
            StringWriter sw = new StringWriter();
            PrintWriter pw = new PrintWriter(sw);
            ServletResponseImpl wrappedResponse =
                (ServletResponseImpl) ServletObjectAccess.getStoredServletResponse(response, pw);
    ...
    /* (non-Javadoc)
     * @see org.infoglue.cms.portal.PortletWindowIG#setParameter(java.lang.String, java.lang.Object)
     */
